  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon-sm" onClick={() => navigate(`/admin/meetings/${meeting.id}`)}>
      ...
      {isReadOnly ? (
        <EmptyState
          ...
          action={
            <Button variant="outline" onClick={() => navigate(`/admin/meetings/${meeting.id}`)}>
              Back to Meeting
            </Button>
          }
        />
        ...
        <div className="max-w-xl">
          <AdminMeetingEditorForm editor={editor} onSaved={() => navigate(`/admin/meetings/${meeting.id}`)} />
        </div>
      )}
    </div>
  )
